...
process_year <- function(file_year, cw = crosswalk, file_path) {
  ...
  df <- read_csv(
    file = file_path,
    col_types = cols(.default = col_character())   # force all to character
  )
  ...
  df <- df %>%
    ...
    mutate(across(everything(), ~stringi::stri_replace_all_regex(.x, pattern = "[\\p{C}]", replacement = ""))) %>%
